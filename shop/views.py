...
def searchMatch(query , item):
    '''Return true only if the reeuired item / product is found in the list of items in My Awesome Cart'''
...
def search(request):
    ...
    if len(allProds) == 0 or len(query)<4:
        params={
            'msg':'Please make sure to enter relevabnt search query'
        }
...
def checkout(request):
    if request.method=="POST":
        ...
        return render(request,  'shop/checkout.html', {'thank':thank, 'id': id})   # {,'shop/checkout.html'param_dict': param_dict})
